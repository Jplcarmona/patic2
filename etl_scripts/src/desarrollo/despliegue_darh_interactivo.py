...
with tab1:
    ...
    if "weather_condition" in df_filtered and not df_filtered["weather_condition"].isna().all():
        ...
        fig = px.bar(
            ...
            title="Ventas por Día y Clima",
            labels={"sales": "Ventas Totales", "day_name": "Dia de la Semana", "weather_condition": "Clima"},
            hover_data=["menu_item_name", "special_event"],
            barmode="group",
        )
        
        fig.update_layout(template="plotly_white", xaxis_title="Día de la Semana", yaxis_title="Ventas ($)")
...
with tab2:
    ...
    model = SARIMAX(df_total["sales"], exog= exog_vars, order=(2, 1, 2), seasonal_order=(1, 1, 1, 7), enforce_stationarity=False, enforce_invertibility=False # ciclo semanal (7 días)
    )
